...
1. 안저이미지 학습 알고리즘 성능 평가: 실제 환자들의 안저 이미지를 활용해 안구질환 이미지를 분류해 내는 AI 개발, 안구질환 데이터는 황반변성, 당뇨성망막병증, 망막정맥폐쇄 이미지로 구성되며 본 대회에서는 안구질환 종류를 예측하는 모델 개발
2. 유방촬영이미지 학습 알고리즘 성능 평가: 실제 환자달의 유방조직 영상으로 악성,양성 이미지로 구성되어 있음. 본 대회에서는 병변이 발견된 조직 부위를 예측하는 모델 개발 
